...
- [Solution](#solution)
  ...
  - [How AI was used, end to end](#how-ai-was-used-end-to-end)
...
## How AI was used, end to end

The individual use cases above and the [ADRs](ADRs/) they cite are the detailed record; this section is the connecting narrative — how the team actually reasoned through AI adoption, quantum by quantum, as one continuous decision rather than four unrelated features.

The Countess's estate had no ticketing, no visitor tracking, and no animal-monitoring infrastructure — and needed to triple visitor volume in three years without tripling headcount. The team's first decision wasn't *which* AI to use, but *how much reach* to give it: every quantum keeps a deterministic core that works with zero AI involvement, and AI is layered on top as an advisory or automation overlay that can fail, time out, or be swapped without breaking anything underneath it. That single rule — decided before any specific use case — is what makes the rest of the design hold together.

**Visitors.** Ticketing itself stays fully deterministic — capacity is protected by optimistic concurrency control ([ADR-006](ADRs/ADR-006-visitors-ticket-capacity-concurrency.md)) with no model in the loop. AI enters only as a **Family Pass Assistant** that reads a visitor's free-text party description and suggests a bundle; on a timeout or low-confidence response it falls through to a local rule-based recommender, and it's explicitly barred from touching price or checkout ([ADR-007](ADRs/ADR-007-visitors-ai-advisory-boundary.md)). The same reasoning — free text is where AI genuinely beats a rule table — is applied narrowly to visitor feedback: a lightweight classifier flags only "urgent safety issue, yes/no" in real time, deliberately biased toward false alarms, while all deeper sentiment/theming work is left to Analytics rather than duplicated here ([ADR-008](ADRs/ADR-008-visitors-feedback-triage-scope.md)). A related decision keeps anonymous feedback out of Marketing's reach entirely, enforced at the point of publication rather than left to convention ([ADR-009](ADRs/ADR-009-visitors-feedback-event-routing.md)).

**Analytics & Staffing.** Footfall sensor data feeds an AI forecasting agent, but the live heatmap and the AI's hotspot prediction are published as two structurally separate feeds so staff can trust "current" unconditionally and treat "forecast" as reviewable opinion ([ADR-004](ADRs/ADR-004-separate-raw-and-ai-derived-paths.md)), backed by two data stores suited to each access pattern ([ADR-003](ADRs/ADR-003-dual-data-store-strategy.md)). When that forecast turns into a staffing action, a GenAI triage agent drafts the dispatch, but the estate never lets AI move people on its own: low-severity crowd nudges auto-dispatch, while anything involving a heritage ride or an exotic animal blocks on a manager's one-tap approval, with a 45-second auto-escalation if no one's watching ([ADR-013](ADRs/ADR-013-staffing-ai-dispatch-authorization.md)). Those orders reach the field over an offline-first MQTT path so a dead Wi-Fi zone delays a dispatch instead of losing it ([ADR-014](ADRs/ADR-014-staffing-field-app-connectivity.md)).

**Maintenance.** This is where "AI helping" and "AI inventing" had to be pulled apart most carefully. Cameras and sensors on 40 rides and 55 enclosures run inference on-device and publish only compact, decision-ready events — never raw video or waveforms — deliberately tuned to over-flag rather than miss a real problem, and for piranhas specifically, population health comes from biomass and feeding-load sensors rather than unreliable camera counts of fast schooling fish ([ADR-010](ADRs/ADR-010-maintenance-feed-ingestion.md)). Those events survive connectivity gaps through a disk-backed store-and-forward gateway — late is acceptable, lost is not ([ADR-011](ADRs/ADR-011-maintenance-event-delivery.md)). When an anomaly fires, a RAG-based Maintenance Copilot drafts a work order, but only from retrieved passages in the estate's own manuals and vet records, with every step cited; if retrieval comes back weak, it refuses to guess and escalates to a human instead ([ADR-012](ADRs/ADR-012-maintenance-work-order-guidance.md)).

**Marketing.** Personalization is the one quantum with no standalone ADR of its own — it inherits the boundary [ADR-004](ADRs/ADR-004-separate-raw-and-ai-derived-paths.md) sets elsewhere: Marketing subscribes to `VisitCompleted` events and reads visit history from the data lake through a published contract, never reaching into Analytics' internal stores directly. A recommendation model suggests a return offer, but campaign rules and channels (email, SMS, in-app) stay swappable independently of that model, so adding a channel is additive rather than a redeploy. And because a visitor dismissing an offer is published as a first-class event rather than just a UI action, that dismissal becomes exactly the kind of override signal ADR-001's monitoring needs — without it, this quantum would have no way to tell if its recommendations were quietly getting worse.

**Making it safe to run in production.** Two decisions apply the same way across every use case above, rather than each quantum solving vendor risk and drift detection on its own. Every AI call — the ticket assistant, the triage agent, the copilot — goes through one internal AI Gateway, so a provider price hike, outage, or shutdown is a config change, not a rewrite ([ADR-002](ADRs/ADR-002-external-ai-integration.md)). And because a healthy gateway says nothing about whether the AI's *answers* are still good, every AI decision already passes through some human confirm/correct/override action — a keeper confirming a flagged animal, a manager approving a dispatch, a visitor accepting a recommendation — and the **rolling rate of those overrides**, tracked per model version, is the metric that decides if something's silently gone wrong; a large enough rise pages an engineer and rolls the gateway back to the last known-good model ([ADR-001](ADRs/ADR-001-ai-vendor-risk-and-monitoring.md)).

None of this assumes a trained model exists on day one. Every use case starts on a rule or a threshold and only earns a trained model once enough estate-specific data — and enough confirmed overrides beating that baseline — exists to justify it (see [Roadmap](#roadmap)).
