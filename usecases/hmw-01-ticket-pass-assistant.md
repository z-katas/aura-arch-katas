...
- **The recommendation is advisory, never a charge.** The footnote is explicit: "Payment is not taken until you confirm. The Ticket Service issues the pass; the assistant does not charge you." This mirrors [ADR: Separate Raw Telemetry Path from AI-Derived Insight Path](../ADRs/ADR-separate-raw-and-ai-derived-paths.md) — AI-derived output (the recommendation) is structurally and visually separated from the deterministic transaction (the actual ticket issuance).
